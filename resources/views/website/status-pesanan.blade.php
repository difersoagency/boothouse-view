...
@section('content')
        ...
        <div class="w-full lg:w-[1280px] mx-auto px-[20px] lg:px-[80px]  mb-16 mt-14">
            <div>
                <ul class="grid md:grid-cols-6 gap-5">
                    <li>
                        <input type="radio" id="dataRadio" name="ukuran" value="dataRadio" class="hidden peer" required
                            checked="checked">
                        <label for="dataRadio"
                            class="inline-flex justify-between items-center px-5 py-2 rounded-lg border-2 border-prim-dark-green bg-prim-white text-prim-dark-green font-bold cursor-pointer transition-all  peer-checked:bg-prim-light-blue peer-checked:text-prim-dark-green hover:text-prim-dark-green hover:bg-prim-light-blue">
                            <div class="block">
                    ...
                    <li>
                        ...
                        <label for="riwayatRadio"
                            class="inline-flex justify-between items-center text-center py-2 px-5 rounded-lg border-2 border-prim-dark-green bg-prim-white text-prim-dark-green font-bold cursor-pointer transition-all  peer-checked:bg-prim-light-blue peer-checked:text-prim-dark-green hover:text-prim-dark-green hover:bg-prim-light-blue">
                    ...
                    @foreach ($order as $d)
                        <div class="px-4 py-5 bg-prim-light-blue rounded-md grid grid-cols-2 items-center">
                            ...
                            <div class="ket">
                                ...
                                <p class="text-prim-dark-green text-[14px] mt-4 font-bold">
                                    {{ $d->DetailOrder->DetailBooth->JenisBooth->nama }}</p>
                                <p class="text-prim-dark-green text-[14px] mt-4 font-bold">
                                    {{ $d->DetailOrder->DetailBooth->ukuran }} ({{ $d->DetailOrder->warna_booth }})</p>
                                <p class="text-prim-dark-green text-[14px] mt-1 font-bold">Tanggal : <span>
                                        {{ $d->tgl_order }} </span> </p>
                                <p class="text-prim-dark-green text-[14px] mt-1 font-bold">Total : <span>Rp.
                                        {{ number_format($d->total_harga) }}</span> </p>
                                <p class="text-prim-dark-green text-[14px] mt-1 font-bold">
                                    {{ $d->total_harga != $d->Pembayaran->sum('total_bayar') ? 'Belum Lunas' : 'Lunas' }}
                                </p>
                                <button
                                    class="bg-prim-dark-green px-5 mt-3 hover:bg-prim-dark-blue transition-colors rounded-lg py-1  text-prim-white font-bold">
                                    Detail
                                </button>
                            </div>
                        </div>
            ...
            <div class="dataDiri mt-10 w-[50%]">
                ...
                <form action="{{ route('user.update', ['id' => Auth::user()->id]) }}" class="grid grid-cols-2 mt-5"
                    id="profil_update" method="POST">
                    @csrf
                    <div class="mr-7">
                        <label for="email" class="mb-2 text-prim-dark-green font-bold text-[14px]">Email</label> <br>
                        <input type="email" name="email"
                            class="mt-3 h-10 rounded-md border-2 w-full border-bg-prim-dark-green px-3 text-prim-dark-green"
                    ...
                    <div class="mb-5">
                        ...
                        <input type="password" name="password" id="password"
                            class="mt-3 h-10 rounded-md border-2 w-full border-bg-prim-dark-green px-3 text-prim-dark-green"
                            value="{{ Auth::user()->customer->password }}" disabled> --}}
                    </div>
                    <div class="mr-7">
                        <label for="provinsi" class="mb-2 text-prim-dark-green font-bold text-[14px]">Provinsi</label> <br>
                        <select type="text" name="provinsi" id="select-provinsi"
                            class=" mt-3 h-10 rounded-md border-2 w-full border-bg-prim-dark-green px-3 text-prim-dark-green">
                        </select>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <select type="text" name="kota" id="select-kota"
                            class="select2 mt-3 h-10 rounded-md border-2 w-full border-bg-prim-dark-green px-3 text-prim-dark-green">
                        </select>
                    </div>
                    ...
                    <div class="">
                        <label for="tel" class="mb-2 text-prim-dark-green font-bold text-[14px]">No.Telepon</label>
                        <br>
                        <input type="text" name="tel" id="tel"
                            class="mt-3 h-10 rounded-md border-2 w-full border-bg-prim-dark-green px-3 text-prim-dark-green"
                            value="{{ Auth::user()->customer->no_telp }}">
                    </div>
                    <div class="col-span-2">
                        {{-- <div
                            class="editInfo bg-prim-dark-green text-prim-white text-[14px] px-4 py-2 mt-7 mr-3 rounded-xl w-fit cursor-pointer">
                            Ubah Data Diri</div> --}}
                        <button
                            class="saveInfo bg-prim-light-blue text-prim-dark-green text-[14px] px-4 py-2 mt-7 rounded-xl w-fit cursor-pointer"
    ...
    <script>
        ...
        //     document.querySelector('.saveInfo').disabled = false;
        ...
        //     document.querySelector('.saveInfo').disabled = true;
        ...
        var option_provinsi = new Option("{{ Auth::user()->Customer->Kota->Provinsi->nama }}",
            {{ Auth::user()->Customer->Kota->Provinsi->id }});
        option_provinsi.selected = true;
        $("#select-provinsi").append(option_provinsi);
        ...
        $('#select-kota').select2({
            ...
            ajax: {
                ...
                delay: 250,
                type: 'GET',
                url: '/provinsi/' + {{ Auth::user()->Customer->Kota->Provinsi->id }},
                data: function(params) {
                    return {
                        term: params.term,
                    }
        ...
        var option_kota = new Option("{{ Auth::user()->Customer->Kota->nama }}",
            {{ Auth::user()->Customer->kota_id }});
        option_kota.selected = true;
        $("#select-kota").append(option_kota);
        $('#select-kota').trigger('change');




        $(document).on('submit', '#profil_update', function(event) {
            event.preventDefault();
            var action = $(this).attr('action');
            console.log($('#profil_update').serialize());
            $.ajax({
                url: action,
                type: 'POST',
